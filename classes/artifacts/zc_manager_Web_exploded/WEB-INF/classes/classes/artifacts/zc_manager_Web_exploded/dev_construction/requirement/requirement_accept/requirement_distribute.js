...
function initReqDisLayOut(ids){
	...
	function initFileUpload(record){
		...
		//初始化附件列表（需求申请书列表）
		getFtpFileList(tablefile, viewFileInfo, record.FILE_ID, "0101");
		//业务需求说明书列表
		...
		//初始化附件列表
		getFtpFileList(asstablefile, assviewFileInfo, record.ASSFILE_ID, "0102");
	}
}
