...
function initSendProduceApply(currTab){
	...
	currTab.find("#system_name").click(function() {
		openSystemPop("sendProduceSystemPop",{name:currTab.find("#system_name"),id:currTab.find("#system_id")});
	});
	...
	addfile.click(function(){
				...
				if(resultForAudit.auditExit=="true"){
					...
					baseAjaxJsonp(dev_construction+'sendProduceApply/queryNoSendProduceAssistTaskList.asp?call='+checkcall+'&SID='+SID,result.param,function(item){
						if(item != undefined&&item != null&&item.result == "true"){
							...
							openFileFtpUpload(currTab.find("#file_modal"), tablefile, 'GZ1062',business_code, '12', 'G_DIC_SEND_PRODUCE_FILE', false, true,paramObj);
						}else{
							var mess = item.mess;
							if(mess){
								alert(mess);
	 ...
	 getFtpFileList(tablefile, currTab.find("#file_view_modal"), currTab.find("#business_code").val(), "12");
