...
function initReqDmIncomeCss(){
	...
	var addfile = getCurrentPageObj().find("#reqDM_addfile");
	 ...
	 //需求申请书初始化附件列表
	 getFtpFileList(tablefile, getCurrentPageObj().find("#reqDM_fileview_modal"), business_code, "0101");
	 //业务需求说明书列表
	 var business_code1 = "";
	 business_code1 = getCurrentPageObj().find("#assfile_id_reqDM").val();
	 var tablefile1 = getCurrentPageObj().find("#reqDMASS_tablefile");
	 getFtpFileList(tablefile1, getCurrentPageObj().find("#reqDMASS_fileview_modal"), business_code1, "0102");
}
