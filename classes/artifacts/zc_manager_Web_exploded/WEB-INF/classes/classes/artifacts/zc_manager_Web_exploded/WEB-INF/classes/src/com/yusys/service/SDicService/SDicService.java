...
//			String[] nomust=new String[]{"supmenu_no","memo"};
	...
	public String findItemByDic(HttpServletRequest req,String userid) {
		...
		try {
			if (pmap!=null) {
				List<Map<String, String>> map=sdicdao.findItemByDic(pmap);
				List<Map<String, String>> map1=new ArrayList<Map<String,String>>();
				for (int i = 0; i < map.size(); i++) {//ITEM_CODE,ITEM_NAME,DIC_CODE,IS_DEFAULT
					Map<String, String> map2=new HashMap<String, String>();
					map2.put("ITEM_CODE", map.get(i).get("item_code"));
					map2.put("ITEM_NAME", map.get(i).get("item_name"));
					map2.put("DIC_CODE", map.get(i).get("dic_code"));
					map2.put("IS_DEFAULT", map.get(i).get("is_default"));
					map1.add(map2);
				}
//				String listMap=redisCache.get(pmap.get("dic_code")+IMyCache.dic, null);
//				if (listMap==null||listMap.trim().length()==0) {
					return JsonUtils.listToJson(map1);
//				}
//				return listMap;
			}
		} catch (Exception e) {
