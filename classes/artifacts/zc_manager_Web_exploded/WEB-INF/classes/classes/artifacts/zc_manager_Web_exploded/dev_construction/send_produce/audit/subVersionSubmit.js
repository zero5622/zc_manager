function initAnnualVersionEditEvent(item,type){//item作为新增修改标识
	...
	if("15"==item.VERSIONS_TYPE){//紧急需求隐藏评审附件信息
		...
		review_file.click(function() {
			openFileFtpUpload(getCurrentPageObj().find("#reqviewaAdd_modalfile"), tablefile1, 'GZ1063',file_id, '0101', 'S_DIC_REVIEW_ACC_FILE', false, false, {FILE_DIR:file_id});
		});
		//附件删除
		var delete_file1 = getCurrentPageObj().find("#reviewdelete_file");
		delete_file1.click(function(){
			delFtpFile(tablefile1, file_id, "0101");
		});
		//初始化附件列表
		getFtpFileList(tablefile1, getCurrentPageObj().find("#reqviewAss_fileview_modal"), file_id, "0101");
	}
	//提交
