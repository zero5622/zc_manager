...
currTab.find('#create_person_name_ReqDisQuery').click(function(){
 ...
 function initReqDisQueryTable() {
		...
		getCurrentPageObj().find('#gReqInfoForDistributeTable').bootstrapTable("destroy").bootstrapTable({
					url :dev_construction+"requirement_accept/queryRequirementInfoForAcceptList.asp?req_state="+req_state+"&SID="+SID+"&call="+reqDisQueryCall+"&menu=req_distribute",
					method : 'get', // 请求方式（*）
					...
					columns : [ {
						...
						align : "center",
						width : "11%",
						formatter:function(value,row,index){if(value=="" || value == undefined || value==null){return "需求管理员";}}
					},{
						field : "CREATE_TIME",
						title : "录入时间",
						align : "center",
