...
public class SUserService implements ISUserService {
	...
	public Map<String, String> insertNewUser(HttpServletRequest req,String actorno){
		...
		try{
			//必填参数列表
			String[] must=new String[]{"user_no","user_name","state","org_no_code","user_mail",
					"is_banker","work_no"};
			//非必填的参数列表
			String[] nomust=new String[]{"nick_name","memo","user_post","user_level","user_mobile"};
...
//    @Cacheable(key="'querySUserByNoCache'+#user_no", value="userCache")
	...
	@Override
	public Map<String, String> queryLoginName(HttpServletRequest req, String loginname) {
		Map<String, String> resultMap=new HashMap<String, String>();
		String login_name = RequestUtils.getParamValue(req, "login_name");
		try{
			//页面上的登录名对应数据库中的记录数
			Integer count = Integer.valueOf(sUserdao.queryLoginName(login_name));	
			if(count>0){
				resultMap.put("result", "true");
				return resultMap;
			}else{
