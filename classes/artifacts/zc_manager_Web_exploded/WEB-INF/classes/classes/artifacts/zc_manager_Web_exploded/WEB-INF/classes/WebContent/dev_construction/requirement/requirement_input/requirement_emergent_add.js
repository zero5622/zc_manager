...
function initReqEmId(){
	...
	getCurrentPageObj().find("input[id=req_Put_dept_name]").val(name);
	 ...
	 addemfile.click(function(){
	 	...
	 	openFileFtpUpload(getCurrentPageObj().find("#reqemadd_modalfile"), tablefile, 'GZ1076',business_code, '0101', 'S_DIC_EMREQ_PUT_FILE', false,false, paramObj);
	 });
	 ...
	 getFtpFileList(tablefile, getCurrentPageObj().find("#reqemadd_fileview_modal"), business_code, "0101");
}
...
//	});
	...
	getCurrentPageObj().find('#req_system_name_reqEmAdd').click(function(){
		openTaskSystemPop("emreq_system_pop",{sysno:getCurrentPageObj().find('#req_system_id_reqEmAdd'),sysname:getCurrentPageObj().find('#req_system_name_reqEmAdd')});
	 });
//保存新增
	...
	if(!vlidate($("#gRequirementInfo_emergentAdd"),"",true)){
	 ...
	 /*var filedata = getCurrentPageObj().find("#reqemadd_filetable").bootstrapTable("getData");
	  ...
	  var req_name_reqEmAdd=getCurrentPageObj().find("#req_name_reqEmAdd").val();
	    if(req_name_reqEmAdd.length>30){
	    	alert("需求名称至多可输入30汉字！");
	    	return;
	    }
...
function initReqEmFile(){
	 ...
	 addemfile.click(function(){
		 var paramObj = new Object();
		 paramObj.FILE_DIR = business_code;
	 	openFileFtpUpload(getCurrentPageObj().find("#reqemadd_modalfile"), tablefile, 'GZ1063',business_code, '0101', 'S_DIC_REQ_PUT_FILE', false,false, paramObj);
	 });
