...
function initSubReqInfo(param) {
	...
	baseAjaxJsonp(url, null, function(data){
		if (data != undefined&&data!=null&&data.result=="true") {
			...
			//初始化附件列表
			getFtpFileList(getCurrentPageObj().find("#table_file"), getCurrentPageObj().find("#file_view_modal"), getCurrentPageObj().find("#sub_req_code").html(), "S_DIC_UAT_TEST_REVIEW");
			uploadUatReview();
		}
	},call );
}
...
function uploadUatReview() {
	...
	addfile.click(function(){
		...
		openFileFtpUpload(currTab.find("#file_modal"), tablefile, business_code, "S_DIC_UAT_TEST_REVIEW","S_DIC_UAT_TEST_FILE", paramObj);
	});
	...
	delete_file.click(function(){
		var business_code = currTab.find("#sub_req_code").html();
		delFtpFile(tablefile, business_code, "S_DIC_UAT_TEST_REVIEW", currTab.find("#file_modal"));
	});
}
