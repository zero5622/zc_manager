...
function initSendProduceInstancyApply(currTab){
	...
	currTab.find("#system_name").click(function() {
		openSystemPop("sendProduceInstancySystem",{name:currTab.find("#system_name"),
			id:currTab.find("#system_id"),type:"instancy"});
	});
	//添加紧急任务按钮
	currTab.find("#addInstancyTask").click(function(){ 
		...
		openSendTaskPop("sendProduceSendProPop",{system_id:currTab.find("#system_id"),type:"add"});
	});
	//申请人
		...
		if(result.result == true){
			...
			baseAjaxJsonp(dev_construction+'sendProduceApply/querySendInstancyTaskByTaskIdForCheck.asp?call='+checkcall+'&SID='+SID+
				 ...
				 if(item != undefined&&item != null&&item.result == "true"){
					...
					result.param["approve_status"] = "02";//02:审批中，03：审批通过
	...
	addfile.click(function(){
		...
		openFileFtpUpload(currTab.find("#file_modalInstancy"), tablefile, 'GZ1062',business_code, '12', 'G_DIC_SEND_IPRODUCE_FILE', false, true,paramObj);
	});
	
	 //附件删除
	 var delete_file = currTab.find("#delete_file");
	 delete_file.click(function(){
	 	delFtpFile(tablefile, business_code, "12");
	 });
	 getFtpFileList(tablefile, currTab.find("#file_view_modalInstancy"),business_code, "12");
}
