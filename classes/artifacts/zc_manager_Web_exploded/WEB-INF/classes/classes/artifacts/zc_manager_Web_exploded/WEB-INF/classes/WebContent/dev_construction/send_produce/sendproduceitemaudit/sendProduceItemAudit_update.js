...
function initFtpFileListAndObject1(params, module_flag){
	...
	addfile.click(function(){
			openFileFtpUpload(currTab.find("#file_modal"), tablefile, path_id,business_codes, phase, module_flag, true, true);
	});
	//附件删除
	var delete_file = currTab.find("#delete_file");
	delete_file.click(function(){
		delFtpFile(tablefile, business_codes, phase, currTab.find("#file_modal"));
	});
	//初始化附件列表
	getFtpFileList(tablefile,currTab.find("#file_view_modal"),business_codes, phase);
}
...
function initFtpFileListAndObjectSit(params, module_flag){
	...
	addfile.click(function(){
			openFileFtpUpload(currTab.find("#file_sit_modal"), tablesitfile, path_id,business_codes, phase, module_flag, true, true);
	});
	//附件删除
	var delete_file = currTab.find("#delete_sit_file");
	delete_file.click(function(){
		delFtpFile(tablesitfile, business_codes, phase, currTab.find("#file_sit_modal"));
	});
	//初始化附件列表
	getFtpFileList(tablesitfile,currTab.find("#file_sit_view_modal"),business_codes, phase);
}
