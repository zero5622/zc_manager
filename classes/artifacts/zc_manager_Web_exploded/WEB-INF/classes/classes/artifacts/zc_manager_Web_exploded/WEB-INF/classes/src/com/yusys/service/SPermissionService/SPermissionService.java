...
//		}
	...
	public String getUserMenu(String userno){
		List<Map<String, String>> userRoles=permissionDao.queryUserRoles(userno);
		List<Map<String, String>> meuns=getMenus(userRoles);
		return (userRoles==null||userRoles.size()==0)?"[]":JsonUtils.beanToJson(getUserMenuMap(meuns));
	}
	/**
	 * 根据用户编号生成用户权限
	 * @param userno
...
//			}
	...
	private String getMenuActions(List<Map<String, String>> userRoles){
		...
		for (int i = 0; i < roleMenuActions.size(); i++) {
			...
			actionCount.put(roleMenuActions.get(i).get("MENU_NO")+"_"+roleMenuActions.get(i).get("ACTION_NO"),count);
