...
function initReqUpdateLayout(ids){
	...
	baseAjaxJsonp(dev_construction+"requirement_input/queryRequirementInfoByID.asp?SID="+SID+"&req_id="+ids+"&call="+reqUpdateCall, null , function(data) {
		...
		//初始化附件列表
		getFtpFileList(tablefile,getCurrentPageObj().find("#reqUpdate_fileview_modal"), data.FILE_ID, "0101");
		getFtpFileList(getCurrentPageObj().find("#reqAssUpdate_filetable"),getCurrentPageObj().find("#reqAssUpdate_fileview_modal"), data.ASSFILE_ID, "0102");
		record = data;
	},reqUpdateCall);
		...
		if(!vlidate($("#gRequirementInfo_update"),"",true)){
	        ...
	        var rid=getCurrentPageObj().find('#req_income_doc_reqUpdate').attr("placeHolder");
		      ...
		      var req_name_reqUpdate=getCurrentPageObj().find("#req_name_reqUpdate").val();
			    if(req_name_reqUpdate.length>30){
			    	alert("需求名称至多可输入30汉字！");
			    	return;
			    }
			...
			baseAjaxJsonp(dev_construction+"requirement_input/updateRequirementInfo.asp?SID="+SID+"&req_state=01"+"&p_owner="+SID, params , function(data) {
	...
	function updateToSubmit(){
	    ...
	    baseAjaxJsonp(dev_construction+"requirement_input/updateRequirementInfo.asp?SID="+SID+"&req_state=02"+"&powner=''", params , function(data) {
			if (data != undefined && data != null && data.result=="true") {
					if (data != undefined && data != null && data.result=="true"){
						alert("更新并提交成功");
						closePageTab("updateRequirement_input");
	...
	addfile.click(function(){
		...
		paramObj.FILE_DIR = record.FILE_ID;
	 	openFileFtpUpload(getCurrentPageObj().find("#reqUpdate_filemodal"), tablefile, 'GZ1063',record.FILE_ID, '0101', 'S_DIC_REQ_PUT_FILE', false, false, paramObj);
	});

	//附件删除
	var delete_file = getCurrentPageObj().find("#reqUpdate_deletefile");
	delete_file.click(function(){
		delFtpFile(tablefile, record.FILE_ID, "0101");
	});
	...
	addfile1.click(function(){
		...
		openFileFtpUpload(getCurrentPageObj().find("#reqAssUpdate_modalfile"), tablefile1, 'GZ1063',record.ASSFILE_ID, '0102', 'S_DIC_REQ_ACC_FILE', false, false, paramObj1);
	});

	//附件删除
	var delete_file1 = getCurrentPageObj().find("#reqUpdate_assDelfile");
	delete_file1.click(function(){
		delFtpFile(tablefile1, record.ASSFILE_ID, "0102");
	});
