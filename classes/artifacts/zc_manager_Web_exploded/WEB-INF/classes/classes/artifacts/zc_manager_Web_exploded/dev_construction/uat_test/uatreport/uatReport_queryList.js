...
function uatReportQueryUrl(){
	var url = dev_construction+'UatReport/queryAllReqSubReport.asp?call='+uatReport_queryList_call+'&SID='+SID+url_type;
	var finds = getCurrentPageObj().find("#reqSubReportTerm [name]");
	for(var i=0; i<finds.length; i++){
		var obj=$(finds[i]);
		if($.trim(obj.val())!=""){
...
(function() {
	...
	getCurrentPageObj().find("#reqSubReportTable").bootstrapTable(
			{
				url : dev_construction+'UatReport/queryAllReqSubReport.asp?call='+uatReport_queryList_call+'&SID='+SID+url_type,
				method : 'get', //请求方式（*）   
