...
function initReqDetailFileList(req_state){
 if(parseInt(req_state)>=9){
	...
	getFtpFileList(tablefile,getCurrentPageObj().find("#reqAPR_fileview_modal"),file_id,"0101");	
	//初始化需求工作量评估信息附件列表
	var ass_tablefile = getCurrentPageObj().find("#reqAssAPR_tablefile");
	getFtpFileList(ass_tablefile,getCurrentPageObj().find("#reqAprAss_fileview_modal"),file_id,"0102"); 
 }else{
   //初始化需求信息附件列表
    ...
    var ass_tablefile = getCurrentPageObj().find("#reqAssAPR_tablefile");
    getFtpFileList(ass_tablefile,getCurrentPageObj().find("#reqAprAss_fileview_modal"),ass_file_id,"0102");
 }
}
