...
currTab.find('#DAAreq_acc_productman_name').click(function(){
	openRoleUserPop("reqAccUserPop",{no:currTab.find('#DAAreq_acc_productman'),name:currTab.find('#DAAreq_acc_productman_name')},"0017");
});

//应用pop
currTab.find('#DAAsystem_name').click(function(){
	openTaskSystemPop("reqAccSysPop",{sysno:currTab.find('#DAAsystem_id'),sysname:currTab.find('#DAAsystem_name')},function(row){
...
currTab.find('#DAAreq_analysis_name').click(function(){
	openRoleUserPop("reqAccUserPop",{no:currTab.find('#DAAreq_analysis'),name:currTab.find('#DAAreq_analysis_name')},"0027");
});
...
function initReqDetailFileList(){
	...
	//初始化需求信息附件列表
	getFtpFileList(reqinfotablefile, getCurrentPageObj().find("#reqinfo_fileview_modal"), reqinfofile_id, "0101");
