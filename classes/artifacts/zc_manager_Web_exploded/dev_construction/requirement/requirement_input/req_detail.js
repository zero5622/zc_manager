...
function initReqDetailLayout(ids){
	...
	baseAjaxJsonp(dev_construction+"requirement_input/queryRequirementInfoByID.asp?SID="+SID+"&req_id="+ids+"&call="+reqdetailCall, null , function(data) {
		...
		if(req_state>=10){
		  getFtpFileList(tablefile, getCurrentPageObj().find("#reqRD_fileview_modal"), data.REQ_CODE, "0101");	
		}else{
		  getFtpFileList(tablefile, getCurrentPageObj().find("#reqRD_fileview_modal"), data.FILE_ID, "0101");
		}
		//业务需求说明书 
		var tablefile2 = getCurrentPageObj().find("#reqAss_detailfile");
		if(req_state>=10){
		  getFtpFileList(tablefile2, getCurrentPageObj().find("#reqAss_detail_modal"), data.REQ_CODE, "0102");	
		}else{
		  getFtpFileList(tablefile2, getCurrentPageObj().find("#reqAss_detail_modal"), data.ASSFILE_ID, "0102");
		}
