			...
			if(data.info){
				...
				apply_del.click(function(){
					if(req_state!="10"&&req_state!="11"&&req_state!="12"&&req_state!="08"){//审批前的状态,按照file_id来删除，加载文档
						delFtpFile(reqApply_tablefile, data.info["input_file_id"].toUpperCase(), "0101");
					}else{//审批后的按照需求编号来删除文档
						delFtpFile(reqApply_tablefile, business_code, "0101");
					}
				});
				...
				if(req_state!="10"&&req_state!="11"&&req_state!="12"&&req_state!="08"){//审批前的状态,按照file_id来删除，加载文档
					apply_add.click(function(){
						...
						if(is_emergency_req !='00'){//一般需求
							openFileFtpUpload(currTab.find("#reqApply_filemudel"), reqApply_tablefile, 'GZ1063',data.info["input_file_id"].toUpperCase(), '0101', dic, false, false, paramObj);
						}else{
							//paramObj.FILE_DIR = business_code;
							openFileFtpUpload(currTab.find("#reqApply_filemudel"), reqApply_tablefile, 'GZ1076',data.info["input_file_id"].toUpperCase(), '0101', dic, false, false, paramObj);
						}
					});
					//初始化文件列表
					getFtpFileList(reqApply_tablefile,currTab.find("#reqApply_filelist"), data.info["input_file_id"].toUpperCase(), "0101");
					...
					apply_add.click(function(){
						...
						openFileFtpUpload(currTab.find("#reqApply_filemudel"), reqApply_tablefile, 'GZ1066',business_code, '0101', dic, false, false, paramObj);
					});
					//初始化文件列表
					getFtpFileList(reqApply_tablefile,currTab.find("#reqApply_filelist"), business_code, "0101");
				}
				...
				assess_del.click(function(){
					if(req_state!="10"&&req_state!="11"&&req_state!="12"&&req_state!="08"){//审批前的状态,按照file_id来删除，加载文档
						delFtpFile(reqAssess_tablefile,data.info["accept_file_id"].toUpperCase(), "0102");	
					}else{//审批后的按照需求编号来删除文档
						delFtpFile(reqAssess_tablefile, business_code, "0102");	
					}
				});
				if(req_state!="10"&&req_state!="11"&&req_state!="12"&&req_state!="08"){//审批前的状态,按照file_id来删除，加载文档
					...
					assess_add.click(function(){
						openFileFtpUpload(getCurrentPageObj().find("#reqApply_filemudel"), reqAssess_tablefile, 'GZ1063',accept_file_id.toUpperCase(), '0102', 'S_DIC_REQ_ACC_FILE', false, false, paramObj);	
					});
					//初始化附件列表
					getFtpFileList(reqAssess_tablefile, getCurrentPageObj().find("#reqAssess_filelist"), accept_file_id.toUpperCase(), "0102");
				}else{//审批后的按照需求编号来删除，加载文档
					...
					assess_add.click(function(){
						...
						openFileFtpUpload(getCurrentPageObj().find("#reqApply_filemudel"), reqAssess_tablefile, 'GZ1067',business_code, '0102', 'S_DIC_REQ_ACC_FILE', false, false, paramObj);	
					});
					//初始化附件列表
					getFtpFileList(reqAssess_tablefile, getCurrentPageObj().find("#reqAssess_filelist"), business_code, "0102");
				}
			}
		}
	}, call_req);
...
function initReq360Btn(){
	...
	getCurrentPageObj().find('#subReqDetail_view').click(function(){
		...
		if(id.length==1){
			...
			baseAjaxJsonp(dev_construction+"requirement_splitTask/querySplitTaskList.asp?SID="+SID+"&call="+call+"&sub_req_id="+sub_req_id+"&limit="+"10"+"&offset="+"0"+"&detail="+detail,null, function(data){
