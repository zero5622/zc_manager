...
function initsitSubmitEditBtn(item){
	...
	baseAjaxJsonp(dev_construction+"GSitSubmit/queryOneSitSubmit.asp?call="+initConfigPageCall+"&SID="+SID+"&sit_id="+item.SIT_ID, null ,function(data){
		if (data != undefined && data != null && data.result=="true" ) {
			...
			//初始化文件列表
			getFtpFileList(currTab.find("#table_file"), getCurrentPageObj().find("#fileview_modal"), items.FILE_ID, "09002");
	...
	addfile.click(function(){
		...
		openFileFtpUpload(currTab.find("#file_modal"), tablefile, 'GZ1059002',business_code, '09002', 'S_DIC_SIT_DEPLOY_FILE', false, true, paramObj);
	});
	 //附件删除
	 var delete_file = currTab.find("#delete_file");
	 delete_file.click(function(){
	 	delFtpFile(tablefile, business_code, "09002");
	 });
