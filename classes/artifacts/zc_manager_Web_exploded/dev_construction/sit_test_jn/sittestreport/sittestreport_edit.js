...
function initSitTestReportInfoLayout(item){
	...
	addfile.click(function(){
		openFileFtpUpload(currTab.find("#file_modal"), tablefile, file_id, "S_DIC_SIT_TEST_FILE", "xq2017-3-13", paramObj);
	});
	
	//附件删除
	var delete_file = currTab.find("#delete_file");
	delete_file.click(function(){
		delFtpFile(tablefile, file_id);
	});
	
	//初始化附件列表
	getFtpFileList(tablefile,currTab.find("#file_view_modal"),file_id);
}
