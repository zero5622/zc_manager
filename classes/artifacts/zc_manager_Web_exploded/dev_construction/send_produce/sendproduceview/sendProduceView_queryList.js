...
function uatSendProduceUrl(){
	var url = dev_construction+'sendProduceApply/queryAllSendProInfo.asp?call='+sendProduceApply_queryList_call+'&VIEW=1&SID='+SID;
	var queryCondition = getCurrentPageObj().find("#queryCondition [name]");
	for(var i=0; i<queryCondition.length; i++){
		var obj=$(queryCondition[i]);
		if($.trim(obj.val())!=""){
...
(function () {
	...
	getCurrentPageObj().find("#sendProduceTable").bootstrapTable({
				url : dev_construction+'sendProduceApply/queryAllSendProInfo.asp?call='+sendProduceApply_queryList_call+'&VIEW=1&SID='+SID,
				method : 'get', //请求方式（*）   
